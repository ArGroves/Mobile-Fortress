...
namespace MobileFortressServer.Character
{
    /// <summary>
    ...
    /// </summary>
    class CharacterController : Updateable, IEndOfTimeStepUpdateable
    {
        /// <summary>
        ...
        public ControlMechanism Controls { get; set; }
        ...
        public void ProcessControls()
        {
            if (Controls != null)
            {
                ...
                Quaternion orientation = Quaternion.CreateFromYawPitchRoll(Controls.Yaw,Controls.Pitch,Controls.Roll);
                Vector3 Forward = Vector3.Transform(Vector3.Forward,orientation);
                Vector3 Left = Vector3.Transform(Vector3.Left,orientation);
                if (Controls.Down)
                {
                    totalMovement += Forward;
                }
                if (Controls.Up)
                {
                    totalMovement -= Forward;
                ...
                {
                    MovementDirection = Vector2.Normalize(new Vector2(totalMovement.X,totalMovement.Z));
                }
            }
        }
    }
